...
const Orders = () => {
  ...
  const isCompleted = (currentStatus, targetStatus) => {
    ...
    return steps.indexOf(currentStatus) >= steps.indexOf(targetStatus);
  };
        ...
        orders.map((order) => (
          <div key={order._id} className="bg-white p-6 rounded-xl shadow mb-6">
            ...
            <div className="space-y-2">
              <p>{isCompleted(order.status, "Payment Verification Pending") ? "✅" : "⭕"} Order Placed</p>
              <p>{isCompleted(order.status, "Confirmed") ? "✅" : "⭕"} Payment Verified</p>
              <p>{isCompleted(order.status, "Shipped") ? "✅" : "⭕"} Shipped</p>
              <p>{isCompleted(order.status, "Delivered") ? "✅" : "⭕"} Delivered</p>
            </div>
